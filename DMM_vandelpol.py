...
def main(args):
    ...
    def saveReconSinGraph(train_data, recon_data, length, path, number):
        ...
        plt.plot(recon_data.detach().numpy()[:,0], recon_data.detach().numpy()[:,1], label="Reconstructed data")
        # plt.plot(recon_data.detach().numpy(), label="Reconstructed data")
    ...
    def saveGeneSinGraph(gene_data, length, path, number):
        ...
        ax = fig.add_subplot()
        # plt.rcParams["font.size"] = FS
        plt.plot(gene_data.detach().numpy()[:,0], gene_data.detach().numpy()[:,1], label="Generated data")
    ...
    for i in range(args.N_songs):
        traing_data.append(musics.VanDelPol(torch.randn(2), args.length, args.T))
